...
class LinkedInBot:
    ...
    def __scrawl_jobs(self, number_of_jobs):
        ...
        for job in jobs:
            ...
            __jobs_scrapped.append([title, company, location, link])

        print(len(jobs), "jobs scrapped")

        print(__jobs_scrapped)

    """Methods that display the list of jobs necessary to scrap the required number of jobs (presses buttons)"""
    ...
    """Methods that save the data in the spreadsheet, not needed when using DB"""
    def save_jobs(self, jobs_array):
